    ...
    class Clear < SlackRubyBot::Commands::Base
      ...
      command 'clear' do |client, data, _match|
        if cache.get('list').nil?
          ...
          client.say(channel: ENV['SLACK_FOOD_CHANNEL_ID'], text: '*Someone is on their way to buy gipfelis.'\
                          'Your order should arrive in the break room or kitchen within the next minutes.*')
